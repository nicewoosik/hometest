...
async function crawlPage(url) {
  ...
  try {
    ...
    $('a[href]').each((i, elem) => {
      ...
      if (resolved && resolved.includes('ecstel.co.kr') && resolved.endsWith('.html')) {
    ...
    $('link[rel="stylesheet"]').each((i, elem) => {
      ...
      const resolved = resolveUrl(href, url)
      if (resolved) {
        const cssPath = resolved.replace(BASE_URL, '').replace(/^\//, '')
        const localPath = path.join(OUTPUT_DIR, cssPath)
        download(resolved, localPath)
      }
    })
    ...
    $('script[src]').each((i, elem) => {
      ...
      if (src && !src.startsWith('data:')) {
        ...
        if (resolved && resolved.includes('ecstel.co.kr')) {
          const jsPath = resolved.replace(BASE_URL, '').replace(/^\//, '')
